...
function lazy_load_query() {
    ...
    $regular_post_args = array(
        ...
        'offset' => 5 * $page,
        'post_status' => 'publish',
        'tag__not_in' => array( $featured_tag_id->term_id ),
    );

    if ($categories && !$tags) {
        $featured_post_args['cats'] = $categories;
        $regular_post_args['cats'] = $categories;
    }

    if ($tags && !$categories) {
        $featured_post_args['tags__in'] = array($tags);
        $regular_post_args['tags__in'] = array($tags);
    }
    ...
    $the_query = new WP_Query( $regular_post_args );
    // The Regular Loop
    if ( $the_query->have_posts() ) {
        $path = dirname(__FILE__) . '/more_content.php';
        while ( $the_query->have_posts() ) {
